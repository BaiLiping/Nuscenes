...
class HostInfo:
    ...
    def fromScene(cls,
                  ...
                  scene_name:str,
                  message:str)->'Interface.HostData':
        '''
        FIELDS utime x y z vx vy vz ax ay az yawrate rotation_vector
        '''
        vehicle_info = nusc_can.get_messages(scene_name, message)
        pose = np.array([(m['utime'], m['pos'][0], m['pos'][1], m['pos'][2],
                          m['vel'][0], m['vel'][1], m['vel'][2],
                          m['accel'][0], m['accel'][1], m['accel'][2],
                          m['rotation_rate'][2],m['orientation']
                          ) for m in vehicle_info])
        host_time_dict = dict()
        for ind in range(pose.shape[0]):
            host_time_dict[pose[ind][0]] = ind
        return pose, host_time_dict
